...
# shipping-service: publish to SQS
module "shipping_service_irsa" {
  source  = "terraform-aws-modules/iam/aws//modules/iam-role-for-service-accounts-eks"
  version = "~> 5.34"

  role_name = "${local.cluster_name}-shipping-service"
  ...
  oidc_providers = {
    eks = {
      provider_arn               = module.eks.oidc_provider_arn
      namespace_service_accounts = ["ecommerce-prod:shipping-service-sa"]
    }
  }
}
